...
output "secret_arns" {
  description = "Secrets Manager ARNs keyed by logical secret name."
...
output "parameter_arns" {
  ...
  value = {
    ...
    k => "arn:${data.aws_partition.current.partition}:ssm:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:parameter${v.name}"
  }
}
